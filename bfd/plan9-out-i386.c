...
#undef P9_ADJOFF
  ...
  /* Resolve D_BRANCH target offsets to indices in progs_all[].
     ...
     IMPORTANT: back values are function-relative and reset to 0 at each
     ATEXT.  A file containing multiple functions would have duplicate back
     values across functions.  To find the correct target we restrict the
     search to instructions belonging to the same function as the branch.
     We do this by finding the ATEXT entry that precedes the branch (that is
     the function start) and searching only within [func_start, func_end).  */
  for (i = 0; i < nprogs_all; i++)
    {
      if (progs_all[i].to.type == P9D_BRANCH)
        {
          ...
          int j, best = -1;
          /* Find the enclosing function: scan backwards for ATEXT. */
          int func_start = 0;
          for (j = i; j >= 0; j--)
            if (progs_all[j].as == P9AS_TEXT)
              { func_start = j; break; }
          /* Find the function end: first ATEXT after func_start (or end). */
          int func_end = nprogs_all;
          for (j = func_start + 1; j < nprogs_all; j++)
            if (progs_all[j].as == P9AS_TEXT)
              { func_end = j; break; }
          /* Search within [func_start, func_end) for back == target_pc. */
